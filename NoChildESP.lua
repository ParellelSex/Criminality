...
local function shapeBox(box)
   ...
   local TL = camera:WorldToViewportPoint(box["Player"].CFrame * CFrame.new(-3,3,0).p)
   local TR = camera:WorldToViewportPoint(box["Player"].CFrame * CFrame.new(3,3,0).p)
   local BL = camera:WorldToViewportPoint(box["Player"].CFrame * CFrame.new(-3,-3,0).p)
   local BR = camera:WorldToViewportPoint(box["Player"].CFrame * CFrame.new(3,-3,0).p)
   local YE = camera:WorldToViewportPoint(box["Player"].CFrame.p)
...
spawn(function()
  RS.RenderStepped:connect(function() -- On my first box esp I was told "- Make sure you put the ESP in render stepped connection or it will look like a 1980's computer is running it" - CornCatCornDog
     ...
     for i, v in ipairs(boxes) do -- You could use next, I have a habit of ipairs
         ...
         if player and player and settings["Visible"] == true and player.Parent == settings["Path"]  then --Without the player being loaded and having a HumanoidRootPart then the esp can't work
             shapeBox(v)
             local _, withinScreenBounds = camera:WorldToScreenPoint(player.Position) --Withinscreenbounds will be if the player is on our screen
