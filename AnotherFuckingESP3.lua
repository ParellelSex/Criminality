...
local function shapeBox(box)
   ...
   local TL = camera:WorldToViewportPoint(box["Player"].Handle1.CFrame * CFrame.new(-3,3,0).p)
   local TR = camera:WorldToViewportPoint(box["Player"].Handle1.CFrame * CFrame.new(3,3,0).p)
   local BL = camera:WorldToViewportPoint(box["Player"].Handle1.CFrame * CFrame.new(-3,-3,0).p)
   local BR = camera:WorldToViewportPoint(box["Player"].Handle1.CFrame * CFrame.new(3,-3,0).p)
   local YE = camera:WorldToViewportPoint(box["Player"].Handle1.CFrame.p)
   -- TL >> BL | TL >> TR | TR >> BR | BL >> BR |
   box[5].Position = Vector2.new(YE.X, YE.Y)
   box[5].Text = box["Player"].Handle1.Name
   -- Left line
...
spawn(function()
  RS.RenderStepped:connect(function() -- On my first box esp I was told "- Make sure you put the ESP in render stepped connection or it will look like a 1980's computer is running it" - CornCatCornDog
     ...
     for i, v in ipairs(boxes) do -- You could use next, I have a habit of ipairs
         ...
         if player and player:FindFirstChild("Handle1") and player.Parent == settings["Path"] and settings["Visible"] == true then --Without the player being loaded and having a HumanoidRootPart then the esp can't work
             shapeBox(v)
             local _, withinScreenBounds = camera:WorldToScreenPoint(player.Handle1.Position) --Withinscreenbounds will be if the player is on our screen
